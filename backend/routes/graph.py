...
def _get_v1_model(model_name: str, svc):
    """Load the V1 model (french_model.pt) which has model.encoder / model.decoder
    as plain tensors (not ParameterLists). Required for correct G* computation."""
...
def compute(G: np.ndarray, N: int, beta: float):
    ...
    if len(active) == 0:
        return _empty_result(N, beta, ncl, modul, meta, labels)
    ...
    print(f"[GRAPH] β={beta:.3f} thr={t1-t0:.2f}s louv={t2-t1:.2f}s viz={t3-t2:.2f}s tot={t3-t0:.2f}s | raw_e={ne} cl={ncl} show={len(nodes)}n/{len(edges)}e")

    return {
        "beta": beta, "n_neurons": int(N), "n_total_edges": int(ne),
...
def _empty_result(N, beta, ncl, modul, meta, labels):
    return {"beta": beta, "n_neurons": int(N), "n_total_edges": 0,
...
def activate(request: ActivateRequest, req: Request):
    ...
    # Per-token cluster activations + per-token node activations
    # Build set of display node IDs for fast lookup
    ...
    mean_act = np.zeros(N, dtype=np.float32)
    for ti in range(T):
        mean_act += act[ti]
    mean_act /= max(T, 1)
    ...
    for ti in range(T):
        ...
        for cid, cm in cmasks.items():
            s = float(row[cm].sum())
            if s > mx: mx = s
            tcl.append({"cluster_id": cid, "activation": round(s, 2),
                        "active_neurons": int((row[cm] > 0).sum())})
        for x in tcl:
            x["normalized"] = round(x["activation"] / max(mx, 1e-6), 4)

        # Per-token node activations using SURPRISE scores
        # surprise = raw_activation / (mean_activation + eps)
        # This highlights neurons that fire MORE than their average
        tok_na = {}
        candidates = []
        for nid in display_node_list:
            v = float(row[nid])
            if v > 0:
                m = float(mean_act[nid])
                surprise = v / (m + 1e-8)  # how much above average
                candidates.append((nid, v, surprise))
        # Sort by SURPRISE (not raw activation) and take top 40
        candidates.sort(key=lambda x: -x[2])
        top_k = candidates[:40]
        if top_k:
            # Normalize surprise scores to 0-1 range
            max_surprise = top_k[0][2]
            for nid, raw_v, surp in top_k:
                tok_na[str(nid)] = round(surp / max_surprise, 4)

        per_tok.append({"token_idx": ti, "byte": tb[ti], "char": chars[ti],
                        "cluster_activations": sorted(tcl, key=lambda c: -c["activation"]),
                        "node_activations": tok_na})
