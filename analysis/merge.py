...
def heritage_probe(model, heritage, dev):
    ...
    for lang_label, text in [("french_input", fr_text), ("portuguese_input", pt_text)]:
        tokens = torch.tensor(
            [list(text.encode("utf-8")[:512])], dtype=torch.long, device=dev)
...
def main():
    ...
    if not a.skip_eval:
        ...
        if a.finetuned and Path(a.finetuned).exists():
            ...
            print(
                f"  French input  -> French neurons {s['french_input_french_pct']:.1f}% | Portuguese neurons {s['french_input_portuguese_pct']:.1f}%")
            print(
                f"  Portuguese input -> French neurons {s['portuguese_input_french_pct']:.1f}% | Portuguese neurons {s['portuguese_input_portuguese_pct']:.1f}%")
            print(
                f"  Routing quality: {s['routing_quality']:.1f}% | Clear separation: {s['clear_separation']}")
            del ft
    ...
    if a.finetuned and Path(a.finetuned).exists():
        ft_ckpt = torch.load(
            a.finetuned, map_location="cpu", weights_only=False)
